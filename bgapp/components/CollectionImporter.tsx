...
export default function CollectionImporter({ initialUsername }: Props) {
  ...
  async function runImport(u: string, p: string) {
    setStatus("loading");
    setMessage("Connessione a BGG… (può richiedere qualche secondo)");
    // The import runs the full collection sync — allow well past the server cap.
    const res = await apiFetch<{
      error?: string; imported?: number; statusChanged?: number;
      wishlistImported?: number; enriched?: number;
    ...
    setStatus("done");
    const parts = [`${data.imported ?? 0} nuovi giochi importati`];
    if ((data.statusChanged ?? 0) > 0) parts.push(`${data.statusChanged} stati aggiornati`);
    if ((data.wishlistImported ?? 0) > 0) parts.push(`${data.wishlistImported} aggiunti ai desiderata`);
    if ((data.enriched ?? 0) > 0) parts.push(`${data.enriched} aggiornati con dati mancanti`);
  ...
  return (
    <>
      <button onClick={startClick} className="btn-secondary text-sm flex items-center gap-2">
            ...
            <div className="p-4 space-y-3">
              ...
              {status === "loading" && (
                <div className="flex items-center gap-2 text-sm" style={{ color: "var(--text-muted)" }}>
                  <Loader2 size={15} className="animate-spin flex-shrink-0" /> {message}
                </div>
              )}
              {status === "done" && (
